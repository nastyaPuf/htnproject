...
class Method:
    def __init__(self, name, params, subtask, task, order):
        self.name = name
        self.task = task
        self.params = params
...
def parse_method(Dom):
    ...
    for i in range(len(text)):
        ...
        if text[i].find(":method") != -1:
            ...
            meth_param = par_in_act(meth_param)
            m = Method(meth_name, meth_param, meth_subtask, meth_task, meth_order)
            methods.append(m)
            meth_name = ""
            meth_task = ""
            meth_subtask = []
            ...
            firpar = text[i].find('(')
            secpar = text[i].find('(', firpar + 1)
            s = Subtask
            s.name = text[i][firpar:secpar - 1]
            s.tasks = text[i][secpar + 1:]
            meth_subtask.append(s)
        elif (order == 1):
            text[i].strip(')').strip('(')
